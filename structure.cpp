...
void game::load_textures() {
    ...
    bug_texture.loadFromFile(prefix + "/images/bug.png");
    carpet_texture.loadFromFile(prefix + "/images/walkable_field.png");
...
void sudoku_game::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    states.transform *= getTransform();
...
void minigame_bugs::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    states.transform *= getTransform();

    sf::Color outline_color = sf::Color(73, 103, 113);

    sf::RectangleShape mg_bugs_carpet(sf::Vector2f(cell_v_size, cell_h_size));
    mg_bugs_carpet.setTexture(&carpet_texture);
    for (int i = 0; i < v_table; ++i) {
        for (int j = 0; j < h_table; ++j) {
            sf::Vector2f position(i * cell_v_size, j * cell_h_size);
            mg_bugs_carpet.setPosition(position);
            target.draw(mg_bugs_carpet, states);
        }
    }

    sf::RectangleShape bug_shape(sf::Vector2f(cell_v_size, cell_h_size));
